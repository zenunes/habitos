CREATE OR REPLACE FUNCTION public.seed_default_rewards_v1(p_user_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY definer
...
BEGIN
  ...
  FROM (
    VALUES
      ...
      ('Cosmético: Moldura do Perfil (Épica)', 300),
...
AS $$
BEGIN
  PERFORM public.seed_default_rewards_v1(p_user_id);
END;
$$;
...
BEGIN
  ...
  PERFORM public.seed_default_rewards_v1(NEW.id);
...
BEGIN
  ...
  LOOP
    PERFORM public.seed_default_rewards_v1(r.id);
  END LOOP;
END;
$$;
